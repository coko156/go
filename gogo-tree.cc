...
Gogo::define_builtin_function_trees()
{
  ...
  // We use __builtin_alloca to turn non escaping heap allocation into
  // stack allocations.
  define_builtin(BUILT_IN_ALLOCA, "__builtin_alloca", NULL,
		 build_function_type_list(ptr_type_node,
					  size_type_node,
					  NULL_TREE),
		 false);
  ...
  if (is_in_heap)
    {
      ...
      tree space =
	gogo->allocate_memory(no->var_value()->type(), size, no->location(),
			      !no->var_value()->escapes_function());
		      ...
		      bool on_stack /* = false */)
{
  ...
  if (on_stack)
    {
     // Use __builtin_alloca to allocate the memory on the stack.
     return build_call_expr(implicit_built_in_decls[BUILT_IN_ALLOCA],
			     1, size);
    }
  else if (this->imported_unsafe_ || type->has_pointer())
    {
