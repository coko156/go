...
void
Statement::perform_escape_analysis(Escape_analysis_info* escape_info)
{
  this->do_perform_escape_analysis(escape_info);
}
...
// Update escape analysis with the information from the go statement.
void
Go_statement::do_perform_escape_analysis(Escape_analysis_info* escape_info)
{
  Call_expression* expr = this->call()->call_expression();
  go_assert(expr != NULL);

  escape_info->process_go_statement(expr->fn(), expr->args());
}
...
// Update escape analysis information for a send statement.
void
Send_statement::do_perform_escape_analysis(Escape_analysis_info* escape_info)
{
  escape_info->process_send(this->channel_, this->val_);
}
