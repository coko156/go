...
Escape_analysis_object*
Temporary_reference_expression::do_get_escape_analysis_object(
    Escape_analysis_info* escape_info)
{
  ...
  if (this->statement_->init() != NULL)
    ...
    return escape_info->get_ea_object_for_temporary_statement(this->statement_,
							      this);
}
...
// Get the escape analysis object that represents a reference to a function
// variable.
...
class Boolean_expression : public Expression
{
 ...
 protected:
  Escape_analysis_object*
  do_get_escape_analysis_object(Escape_analysis_info* escape_info)
  { return escape_info->process_constant(this); }
...
Escape_analysis_object*
String_expression::do_get_escape_analysis_object(
    Escape_analysis_info* escape_info)
{
  return escape_info->process_constant(this);
}
...
class Type_conversion_expression : public Expression
{
 ...
 private:
  ...
  // True if whatever this expression returns escapes the functional scope.
  bool escapes_function_;
};
...
// Traverse a type conversion operator.
...
Type_conversion_expression::do_get_tree(Translate_context* context)
{
  ...
  if (type->interface_type() != NULL || expr_type->interface_type() != NULL)
    ret = Expression::convert_for_assignment(
	      context,type, expr_type, expr_tree, this->location(),
	      !this->escapes_function());
...
class Unsafe_type_conversion_expression : public Expression
{
 ...
 protected:
  Escape_analysis_object*
  do_get_escape_analysis_object(Escape_analysis_info* escape_info)
  { return escape_info->process_unsafe_conversion(this->expr_); }
...
// An dereference operation is treated as field access with index -1.

Escape_analysis_object*
Unary_expression::do_escape_analysis_split_base_index(
    Escape_analysis_info* escape_info,
    int* field_index)
{
  if (this->op_ != OPERATOR_MULT)
...
// Retrieve the escape analysis object corresponding to the result of a call
// expression.

Escape_analysis_object*
Call_expression::do_get_escape_analysis_object(
    Escape_analysis_info* escape_info)
{
  const Named_object* no = this->fn_->func_expression() == NULL
			   ? NULL
			   : this->fn_->func_expression()->named_object();
  return escape_info->process_call(no, this, this->args_,
				   this->result_count() == 0);
}
...
// Get the associated named object if any.

const Named_object*
Call_expression::get_function_object() const
{
  if (this->fn_->func_expression() != NULL)
    return this->fn_->func_expression()->named_object();
...
// Get escape analysis object for a call result.

Escape_analysis_object*
Call_result_expression::do_get_escape_analysis_object(
    Escape_analysis_info* escape_info)
{
   return escape_info->process_call_result(this, this->call_, this->index_);
}
...
// Retrieve the escape analysis object corresponding to a array index expression.

Escape_analysis_object*
Array_index_expression::do_get_escape_analysis_object(
    Escape_analysis_info* escape_info)
{
  // Determine whether the index expression is a reference to an element
  // or a slice.
  if (this->end_ == NULL)
    {
      // Now we must take care that accessing an array element is exactly like
      // a field access, but accessing a slice element is like a dereference
      // followed by a field access.
      // The expression refers to an element of the array. Array contents
      // will be abstracted by a single element (field index -1), much as
      // dereferences. Individual elements are abstracted away.
      if (!this->array_->type()->is_open_array_type())
	{
	  return escape_info->process_field_reference(this, this->array_, -1);
	}
      else
	{
	  return escape_info->process_slice_access(this, this->array_);
	}
  ...
  else
    {
      if (!this->array_->type()->is_open_array_type())
	{
	  // This expression "creates" a slice, in which case we will create a
	  // pointer to the underlying array reference. This is similar
	  // to taking the address of the array.
	  return escape_info->process_slice_creation(this, this->array_);
	}
      else
	{
	  // Creating a slice of another slice is just a reference copy.
	  // FIXME. While this is correct it might be imprecise. It actually
	  // is like a regular assgignment where two different slices point
	  // to the same array. If the address of a slice can not be taken
	  // then this is precise.
	  return this->array_->get_escape_analysis_object(escape_info);
	}
    }
}

// Split the expression into base and field access for the appropriate
// type of expressions.

Escape_analysis_object*
Array_index_expression::do_escape_analysis_split_base_index(
    Escape_analysis_info* escape_info,
    int* field_index)
{
  // Array elements are abstracted into one field.
  *field_index = -1;

  // We only handle here the access of a single element in an array or map.
  if (this->end_ != NULL || this->array_->type()->is_open_array_type())
    {
      Escape_analysis_object* underlying_ref =
	escape_info->process_field_reference(this, this->array_, -1);
      return underlying_ref;
    }

  return this->get_escape_analysis_object(escape_info);
}
...
class String_index_expression : public Expression
{
 ...
 protected:
  Escape_analysis_object*
  do_get_escape_analysis_object(Escape_analysis_info*)
  // FIXME. String index expressions might be string slices.
  { return NULL; }
...
// Retrieve the escape analysis object corresponding to a map index expression.

Escape_analysis_object*
Map_index_expression::do_get_escape_analysis_object(
    Escape_analysis_info* escape_info)
{
  // Map access is treated exactly as slice access.
  // FIXME. Will a map store a reference to the index expression
  // when a map[index] query is performed? If so then it needs to
  // be added here.
  return escape_info->process_slice_access(this, this->map_);
}
...
Escape_analysis_object*
Map_index_expression::do_escape_analysis_split_base_index(
    Escape_analysis_info* escape_info,
    int* field_index)
{
    // Array elements are abstracted into one field.
  *field_index = -1;
  // We only handle here the access of a single element in an array or map.
  Escape_analysis_object* underlying_ref =
    escape_info->process_field_reference(this, this->map_, -1);

  // Whenever an array is indexed it captures a reference to
  // the indexing value.
  Escape_analysis_object* map_index_value_ref =
    escape_info->get_ea_object_for_expression(this->index_);
  // Now add a reference from the map to the index.
  escape_info->field_assignment_rule(underlying_ref, -1, map_index_value_ref);
...
Escape_analysis_object*
Field_reference_expression::do_get_escape_analysis_object(
    Escape_analysis_info* escape_info)
{
  return escape_info->process_field_reference(this, this->expr_,
					      this->field_index_);
}
...
Escape_analysis_object*
Field_reference_expression::do_escape_analysis_split_base_index(
    Escape_analysis_info* escape_info,
    int* field_index)
{
  *field_index = this->field_index_;
  return this->expr_->get_escape_analysis_object(escape_info);
}
...
Escape_analysis_object*
Interface_field_reference_expression::do_get_escape_analysis_object(
    Escape_analysis_info* escape_info)
{
   return escape_info->process_interface_field_reference(this, this->expr_,
							 this->name_);
}
...
class Allocation_expression : public Expression
{
 public:
  Allocation_expression(Type* type, source_location location)
    : Expression(EXPRESSION_ALLOCATION, location),
      type_(type), escapes_function_(true)
  { }

 protected:
  Escape_analysis_object*
  do_get_escape_analysis_object(Escape_analysis_info* escape_info)
  { return escape_info->process_allocation(this, this->location()); }
 ...
 private:
  ...
  Type* type_;
  // True if this expression escapes its functional scope.
  bool escapes_function_;
};
...
class Struct_construction_expression : public Expression
{
 ...
 protected:
  Escape_analysis_object*
  do_get_escape_analysis_object(Escape_analysis_info* escape_info)
  { return escape_info->process_construction(this, this->vals_, false); }
...
class Array_construction_expression : public Expression
{
 protected:
  Array_construction_expression(Expression_classification classification,
    ...
    : Expression(classification, location),
      type_(type), vals_(vals), escapes_function_(true)
  { }
 ...
 protected:
  Escape_analysis_object*
  do_get_escape_analysis_object(Escape_analysis_info* escape_info)
  { return escape_info->process_construction(this, this->vals_, true); }
...
class Open_array_construction_expression : public Array_construction_expression
{
 ...
 protected:
  Escape_analysis_object*
  do_get_escape_analysis_object(Escape_analysis_info* escape_info)
  { return escape_info->process_container_construction(this, this->vals()); }
...
Open_array_construction_expression::do_get_tree(Translate_context* context)
{
  ...
  if (!copy_to_heap)
  ...
  else
    {
      ...
      space =
	context->gogo()->allocate_memory(element_type, memsize, this->location(),
					 !this->escapes_function());
      space = save_expr(space);
...
class Map_construction_expression : public Expression
{
 ...
 protected:
  Escape_analysis_object*
  do_get_escape_analysis_object(Escape_analysis_info* escape_info)
  { return escape_info->process_container_construction(this, this->vals_); }
...
Escape_analysis_object*
Type_guard_expression::do_get_escape_analysis_object(
    Escape_analysis_info* escape_info)
{
  // Type guard is exactly like a type cast when type is an interface type and
  // we deal especially with guards that involve unsafe pointers..
  if (this->type_->is_unsafe_pointer_type())
    // FIXME. Is an expression e.(unsafe.Pointer) equivalent to an
    // unsafe cast?
    // An unsafe.Pointer type guard is treated as an unsafe type cast.
    return escape_info->process_unsafe_conversion(this->expr_);
 ...
 public:
  Heap_composite_expression(Expression* expr, source_location location)
    : Expression(EXPRESSION_HEAP_COMPOSITE, location),
      expr_(expr), escapes_function_(true)
  { }
...
Heap_composite_expression::do_get_tree(Translate_context* context)
{
  ...
  tree space =
    context->gogo()->allocate_memory(this->expr_->type(), expr_size,
				     this->location(),
				     !this->escapes_function());
...
// Get escape analysis object for a receive expression.

Escape_analysis_object*
Receive_expression::do_get_escape_analysis_object(
    Escape_analysis_info* escape_info)
{
   return escape_info->process_receive(this, this->channel_);
}
...
class Struct_field_offset_expression : public Expression
{
 ...
 protected:
  // FIXME. This should evaluate to a reference to a field reference.
  Escape_analysis_object*
  do_get_escape_analysis_object(Escape_analysis_info* escape_info)
  { return escape_info->process_constant(this); }
